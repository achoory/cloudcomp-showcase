...
var reqStr='';
var result;
connection.query('SELECT * FROM resale_prices.resale_hdb where lat is NULL AND lon is NULL LIMIT 1;', function(err, rows, fields) {
  if (err) throw err;
  if (recordId==null){
    console.log('no records...');
    process.exit();
  }
  var stName=" "+rows[0].street_name+" ";
  stName= stName.replace(" AVE ", " AVENUE ");
  stName= stName.replace(" ST ", " STREET ");
  stName= stName.replace(" RD ", " ROAD ");
  stName= stName.replace(" HT ", " HEIGHT ");
  stName= stName.replace(" PK ", " PARK ");
  stName= stName.replace(" CRES ", " Crescent ");
  stName= stName.replace(" CTRL ", " CENTRAL ");
  stName= stName.replace(" CTR ", " CENTRE ");
  stName= stName.replace(" NTH ", " NORTH ");
  stName= stName.replace(" STH ", " SOUTH ");
  stName= stName.replace(" DR ", " DRIVE ");
  stName= stName.replace(" GDNS ", " GARDENS ");
  stName= stName.replace(" JLN ", " JALAN ");
  stName= stName.replace(" TER ", " Terrace ");
  stName= stName.replace(" BT ", " BUKIT ");
  stName= stName.replace(" UPP ", " UPPER ");
  stName= stName.replace(" C'WEALTH ", " Commonwealth ");
  stName= stName.replace(" TG ", " TANJONG ");
  stName= stName.replace(" LOR ", " LORONG ");
  stName= stName.replace(" ST. ", " SAINT ");
  stName= stName.replace(" CL ", " CLOSE ");
  stName= stName.replace(" HTS ", " HEIGHTS ");
  stName= stName.replace(" KG ", " KAMPONG ");
  stName= stName.replace(" MKT ", " MARKET ");



  address=rows[0].block_n+' '+stName.trim();

  recordId=rows[0].id;
  console.log('Processing id:'+recordId+' Address:'+address);
    reqStr='http://www.onemap.sg/API/services.svc/basicSearch?'
    +'token='+token
    +'&searchVal='+address
    +'&otptFlds=SEARCHVAL,CATEGORY&returnGeom=1&rset=1';
    //console.log(reqStr);
    client.get(reqStr, function (data, response) {
      ...
      var udqry='';
      //console.log(rObj.SearchResults[1]);
      if (rObj.SearchResults[1]==null){
        console.log('no coordinates...');
        udqry='UPDATE resale_prices.resale_hdb set lat="NR", lon="NR", x="NR", y="NR" Where id='+recordId+';'
        //process.exit();
      }else{
        var cv = new SVY21();
        result = cv.computeLatLon(rObj.SearchResults[1].Y,rObj.SearchResults[1].X);
        console.log('Coordinates obtained:'+result.lat+','+result.lon);
        udqry='UPDATE resale_prices.resale_hdb set lat="'+result.lat+'", lon="'+result.lon
        +'", x="'+rObj.SearchResults[1].X
        +'", y="'+rObj.SearchResults[1].Y
        +'" Where id='+recordId+';'
      }
      // raw response
      ...
      //console.log(udqry);
      connection.query(udqry, function(err, rows, fields) {
        if (err) throw err;
      console.log('Record id:'+recordId+' Updated!');
        process.exit();
      });
