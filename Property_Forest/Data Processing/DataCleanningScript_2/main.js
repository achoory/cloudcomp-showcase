...
connection.query('SELECT * FROM resale_prices.schools where lat is NULL AND lon is NULL LIMIT 1;', function(err, rows, fields) {
  if (err) throw err;
  recordId=rows[0].id
  if (recordId==null){
  ...
  client.get(reqStr, function (data, response) {
    ...
    if (rObj.SearchResults[1]==null){
      ...
      udqry='UPDATE resale_prices.schools set lat="NR", lon="NR", x="NR", y="NR" Where id='+recordId+';'
      //process.exit();
    }else{
      var cv = new SVY21();
      ...
      udqry='UPDATE resale_prices.schools set lat="'+result.lat+'", lon="'+result.lon
